# Scripts for reproducing our paper's results

Scripts are all saved in Jupyter notebooks (which should be viewable directly on GitHub), or in files such as `script` or `README`. To reproduce results in section 3.4 (ultra-compress data), the same scripts in 3.2 (error & timing & memory benchmarks) can be used: simply change all `outfile` names from ending in `.vcf.gz` to `.jlso`. This is described [here](https://openmendel.github.io/MendelImpute.jl/dev/man/ultra+compress/) in our documentation.
