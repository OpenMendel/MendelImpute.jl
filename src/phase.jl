...
function phase(
    ...
    # import (compressed) reference panel
    ref_import_start = time()
    compressed_Hunique = import_compressed_reference(reffile)
    ref_import_time = time() - ref_import_start

    # import genotype data
    genotype_import_start = time()
    X, X_sampleID, X_chr, X_pos, X_ids, X_ref, X_alt = import_target(tgtfile, dosage)
