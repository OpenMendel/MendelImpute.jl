...
"""
Helper function that calculates the optimal subtree starting with `happair` 
as root node in window `w`. 

# Inputs
- `solution_path`: Optimal sequence of haplotype pairs in each window
- `memory`: Vector of dictionary storing the optimal error for each haplotype pair in each window
- `path_err`: Error for each window induced by `sol_path`
- `haplotype_set`: A vector of vectors. `haplotype_set[1]` stores all pairs of haplotypes in window 1 in a vector, and so on. 
- `λ`: Error each switch contributes. Defaults to 1.0
- `w`: Current window where `happair` is in. 
- `happair`: Haplotype pair in current window being considered. 

# Output
- `best_err`: Minimum error induced by subtree generated by `happair` in window `w`. 
"""
